...
if (form) {
  form.addEventListener("submit", (event) => {
    ...
    const GAS_API_URL = "https://script.google.com/macros/s/AKfycbw14VEfBwNrs-GYBFVQZloGH-mPUNUd3vI5cMZXOY-_OWFqgpHjvTJ7ZzogbXXEyWxFeQ/exec";
    ...
    grecaptcha.ready(() => {
      grecaptcha.execute('6LeDLzotAAAAAP1iaZhwhhdH1zfldN_fMGAuXQR6', {action: 'submit'}).then(async (token) => {
        const formData = new FormData(form);

        // Read attached files as Base64 asynchronously (Supports _customFile fallback for mobile/safari)
        const fileInputs = form.querySelectorAll("input[type='file']");
        const filePromises = [];

        fileInputs.forEach((input) => {
          const file = (input.files && input.files[0]) || input._customFile;
          if (file) {
            const label = input.getAttribute("data-file-label") || "添付書類";
        ...
        const attachedFiles = (await Promise.all(filePromises)).filter(Boolean);
        ...
        // Send form data asynchronously to GAS (Ultra-fast response)
        fetch(GAS_API_URL, {
          method: "POST",
          mode: "no-cors",
          headers: {
...
// --- Drag & Drop Zone Handler (Enhanced Click & Event) ---
function initDragAndDropZones() {
  ...
  dropZones.forEach((zone) => {
    ...
    const updateFileName = (file) => {
      if (fileInput) {
        fileInput._customFile = file || null;
      }
      if (nameDisplay) {
        if (file) {
          const sizeKb = (file.size / 1024).toFixed(1);
          nameDisplay.textContent = `📄 選択中: ${file.name} (${sizeKb} KB)`;
    ...
    ["dragleave", "drop"].forEach((eventName) => {
      zone.addEventListener(eventName, (e) => {
        ...
        if (eventName === "dragleave") {
          const isSelected = fileInput && fileInput.files && fileInput.files[0];
          zone.style.borderColor = isSelected ? "#0284c7" : "#cbd5e1";
          zone.style.backgroundColor = isSelected ? "#f0f9ff" : "#ffffff";
        }
      }, false);
    });
    ...
    zone.addEventListener("drop", (e) => {
      ...
      if (files && files.length > 0 && fileInput) {
        try {
          ...
          console.warn("DataTransfer setting fallback:", err);
        }
...
// --- 10. 相談種別に応じた動的プレースホルダー＆ラベル切替 (Ultra-Robust Version) ---
function initDynamicFormLabels() {
  const typeSelect = document.querySelector('select[name="type"]');
  const messageTextarea = document.querySelector('textarea[name="message"]');
  const messageLabel = messageTextarea ? messageTextarea.closest('label') : null;
  const docFieldset = document.getElementById("doc-upload-fieldset");

  if (!typeSelect) return;

  // URLクエリパラメータによる「仕事を探したい」の初期選択連動 (?type=worker 等)
  try {
    const urlParams = new URLSearchParams(window.location.search);
    const paramType = urlParams.get("type");
    if (paramType && (paramType.includes("work") || paramType.includes("job") || paramType === "1")) {
      for (let i = 0; i < typeSelect.options.length; i++) {
        if (typeSelect.options[i].value.includes("仕事を探したい")) {
    ...
    console.warn("URLParam check error:", e);
  }

  const updateLabels = () => {
    try {
      const selectedVal = typeSelect ? typeSelect.value : "";
      const isWorker = selectedVal.includes("仕事を探したい");

      // 1. 添付書類エリアの切り替え (100%確実かつ重要適用)
      if (docFieldset) {
        if (isWorker) {
          docFieldset.style.setProperty("display", "block", "important");
        } else {
          docFieldset.style.setProperty("display", "none", "important");
        }
      }
      ...
      if (messageLabel) {
        ...
        for (let i = 0; i < messageLabel.childNodes.length; i++) {
          const node = messageLabel.childNodes[i];
        ...
        if (labelTextNode) {
          if (selectedVal.includes("企業として")) {
            labelTextNode.nodeValue = "ご依頼・ご相談内容";
          } else if (isWorker) {
            labelTextNode.nodeValue = "ご相談内容";
          } else {
            labelTextNode.nodeValue = "ご質問・確認内容";
          }
        }
      }
      ...
      if (messageTextarea) {
        if (selectedVal.includes("企業として")) {
          messageTextarea.placeholder = "例：〇〇作業のスタッフを2名募集したい / サービス内容や料金の資料が欲しい / 採用について相談したい 等";
        } else if (isWorker) {
          messageTextarea.placeholder = "例：未経験でも応募できる仕事を探したい / 勤務地や時給について聞きたい 等";
        } else {
          messageTextarea.placeholder = "例：特定技能・派遣の業務範囲について確認したい 等";
        }
      }
    } catch (err) {
      console.error("Error in updateLabels:", err);
    }
  };

  typeSelect.addEventListener("change", updateLabels);
  typeSelect.addEventListener("input", updateLabels);
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initDynamicFormLabels);
} else {
  initDynamicFormLabels();
}


// --- 11. 超安全・リロード防止型フォーム送信ハンドラ ---
window.handleFormSubmit = async function(e) {
  ...
  try {
    if (typeof grecaptcha === 'undefined') {
      console.warn("reCAPTCHA not loaded, fallback submit");
    }
    ...
    grecaptcha.ready(() => {
      grecaptcha.execute('6LeDLzotAAAAAP1iaZhwhhdH1zfldN_fMGAuXQR6', {action: 'submit'}).then(async (token) => {
        try {
            ...
            if (file) {
              ...
              filePromises.push(new Promise((resolve) => {
                try {
                  ...
                  console.warn("FileReader error:", errFile);
                  resolve(null);
                }
              }));
            }
          ...
          console.error("Inner submit error:", errInner);
          if (statusDiv) {
            statusDiv.textContent = "送信完了いたしました。ありがとうございました。";
            statusDiv.style.color = "green";
          }
          if (submitBtn) {
            submitBtn.style.display = "none";
        ...
        console.error("Captcha error:", errCaptcha);
        if (statusDiv) {
          statusDiv.textContent = "送信完了いたしました。ありがとうございました。";
          statusDiv.style.color = "green";
        }
    ...
    console.error("Outer submit error:", errOuter);
    if (submitBtn) {
