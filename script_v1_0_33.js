...
function initDynamicFormLabels() {
  ...
  if (!typeSelect || !messageTextarea) return;
  ...
  const updateLabels = () => {
    ...
    // 求職者（仕事を探したい）の時のみ、添付書類エリアを表示
    if (docFieldset) {
      docFieldset.style.display = isWorker ? "block" : "none";
    }

    if (selectedVal.includes("企業として")) {
      if (messageLabel && messageLabel.childNodes.length > 0) {
        messageLabel.childNodes[0].nodeValue = "ご依頼・ご相談内容";
      }
      messageTextarea.placeholder = "例：〇〇作業のスタッフを2名募集したい / サービス内容や料金の資料が欲しい / 採用について相談したい 等";
    } else if (isWorker) {
      if (messageLabel && messageLabel.childNodes.length > 0) {
        messageLabel.childNodes[0].nodeValue = "ご相談内容";
      }
      messageTextarea.placeholder = "例：未経験でも応募できる仕事を探したい / 勤務地や時給について聞きたい 等";
    } else {
      if (messageLabel && messageLabel.childNodes.length > 0) {
        messageLabel.childNodes[0].nodeValue = "ご質問・確認内容";
      }
